...
public class MinioFSService : IMinioFSService
{
    ...
    public MinioFSService(IConfiguration configuration, ILogger<MinioFSService> logger)
    {
        ...
        string minioUser = configuration["MINIO_USER"] ?? "minioadmin200";
        string minioPassword = configuration["MINIO_PASSWORD"] ?? "minioadmin";
        string minioEndpoint = configuration["MINIO_ENDPOINT"] ?? "localhost:9000";
