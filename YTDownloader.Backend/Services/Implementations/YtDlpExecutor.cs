...
using YTDownloader.Backend.Extensions;
...
public class YtDlpExecutor : IYtDlpExecutor
{
    ...
    public YtDlpExecutor(ILogger<YtDlpExecutor> logger, IConfiguration configuration)
    {
        ...
        // Look for yt-dlp.exe in project root
        var projectRoot = Directory.GetCurrentDirectory();
        var ytDlpExePath = Path.Combine(projectRoot, "yt-dlp.exe");

        // Check if yt-dlp.exe exists in project root
        if (File.Exists(ytDlpExePath))
        {
            _ytDlpPath = ytDlpExePath;
            _logger.LogInformation("✓ Using yt-dlp.exe from project root: {Path}", ytDlpExePath);
        }
        else
        {
            _logger.LogWarning("⚠ yt-dlp.exe not found in project root. Using: {Path}", _ytDlpPath);
            _logger.LogWarning("⚠ Expected location: {ExpectedPath}", ytDlpExePath);
        }
        ...
        try
        {
            if (!Directory.Exists(_downloadBasePath))
            {
                ...
                _logger.LogInformation("✓ Download directory created: {Path}", _downloadBasePath);
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task<DownloadResponse> DownloadVideoAsync(DownloadRequest request)
    {
        try
        {
            ...
            _logger.LogInformation("Downloading: {Url}", request.Url);
            ...
            _logger.LogInformation("✓ Download completed: {FileName}", Path.GetFileName(downloadedFile));
            return new DownloadResponse
            {
                Success = true,
